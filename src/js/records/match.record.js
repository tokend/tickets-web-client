...
export class MatchRecord extends TxRecord {
  constructor (record, asset) {
    ...
    // following works only for art:
    this.fromAccount = this._getAccountFrom()
    this.toAccount = this._getAccountTo()
    this.fromEmail = ''
    this.toEmail = ''
  }
  ...
  _getAccountFrom () {
    return this._record.source_account
  }

  _getAccountTo () {
    return ((this.participants || [])
      .find(p => p.account_id !== this._record.source_account) || {})
      .account_id
  }

  _getEffects () {
    return get(this, 'participant.effects', [])
  }
...
export class MatchTransaction {
  constructor (effect, opts) {
    ...
    this.name = this.direction === 'in' ? 'Sell' : 'Buy'
  }
