<template>
  <div class="create-bid">
    ...
    <template v-else-if="ticket.code">
      <div class="create-bid__wrp">
        <div class="create-bid__ticket-img-wrp">
          <img :src="ticket.splashUrl" alt="">
        </div>

        <div class="create-bid__content">
          <h2 class="app__page-heading">
            {{ 'ticket_open_a_new_bid' | translate }}
            <br>
            <span class="create-bid__available-caption">
              {{ 'tickets_bid_available' | translate }}
              {{ i18n.n(available) }}
              {{ ticket.code }}
            </span>
            ...
            <div class="app__form-row">
              ...
              <div class="app__form-field">
                <input-field-unchained name="amount"
                                       ...
                                       v-validate="'required|amount|decimal:0'"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isNeededToIssue () {
        return +_get(this.accountBalances, `${this.ticket.code}.balance`) === 0 &&
          this.ticket.owner === this.accountId
      },
      available () {
        if (this.isNeededToIssue) {
          return this.ticket.available
        }
        return this.accountBalances[this.ticket.code].balance
      }
    },
    methods: {
      ...mapActions({
      ...
      async loadTicket (code) {
        ...
        this.ticket = this.tickets.find(t => t.code === code)
        ...
        try {
          if (this.isNeededToIssue) {
            await issuanceService.createIssuanceRequest({
              token: this.ticket.code,
              amount: this.ticket.available,
              receiver: this.accountBalances[this.ticket.code].balance_id,
          ...
          await bidsService.createBid({
            balanceID: this.accountBalances[this.ticket.code].balance_id,
          ...
          EventDispatcher.dispatchShowSuccessEvent(i18n.ticket_opened())
          this.$router.push({
