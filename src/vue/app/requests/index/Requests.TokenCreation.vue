            ...
            <md-table-cell colspan="7">
              <md-card-content class="md-layout md-gutter">
                <div class="icon-column md-layout-item md-size-35 md-layout md-alignment-center-center">
                  <router-link :to="{name: 'tickets.ticket-details', params: { id: item.tokenCode }}">
                  <img class="token-icon" v-if="item.iconUrl" :src='item.iconUrl' :alt="documentTypes.tokenIcon">
                  <div class="token-icon" v-else>{{ item.reference.substr(0, 1).toUpperCase() }}</div>
                  </router-link>
                </div>
                <div class="details-column md-layout-item">
                  <detail prop="Request type" :value="`${getFancyName(item.requestType)}`"/>
                  ...
                  <detail prop="Token name" :value="`${item.details.name}`"/>
              ...
              <md-card-actions>
                ...
                <router-link :to="{name: 'ticket-creation.index', params: { id: item.id }}"
                             tag="button"
                             v-ripple
                             class="app__button-flat"
