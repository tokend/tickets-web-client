        ...
        <div class="ticket-market__form-wrp">
          ...
          <div class="ticket-market__available-wrp">
            <p class="ticket-market__available-msg">
              {{ 'ticket_available' | translate }}:
              {{ i18n.n(bid.availableAmount) }} {{ bid.baseAsset }} tickets
            </p>
          </div>

          <div class="ticket-market__field-wrp">
            <input-field-unchained class="md-layout-item"
                                   name="quote-amount" type="number" step="1" min="1"
                                   v-model.trim="form.amount"
                                   v-validate="`required|amount|decimal:0|max_ticket_amount:${bid.availableAmount}`"
    ...
    <md-dialog :md-active.sync="isDialogOpened">
      <div class="app__dialog-inner">
        <template v-if="succeededRequest">
          <swap-address :request="succeededRequest" @close="isDialogOpened = false"/>
        </template>
...
<script>
  ...
  import QuoteViewer from './Tickets.QuoteViewer'
    ...
    methods: {
      ...
      async tryDeriveAddress (requestId) {
        ...
        while (!request.address) {
          ...
          response = await reviewableRequestsService.loadReviewableRequestById(requestId)
          request = RecordFactory.createASwapRequestRecord(response)
        }
