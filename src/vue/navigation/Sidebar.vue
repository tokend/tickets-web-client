    ...
    <section class="sidebar__list">
      <ul>
        <router-link v-ripple class="sidebar__list-item"
                     ...
                     to="/tickets"
                     tag="li" v-if="config.FEATURE_FLAGS.tickets">
          <md-icon class="sidebar__list-item-icon">toll</md-icon>
          <span class="md-list-item-text">{{ i18n.sidebar_link_tickets() }}</span>
        </router-link>

        <router-link v-ripple class="sidebar__list-item"
                     @click.native="closeSidebar"
                     to="/dashboard"
                     tag="li" v-if="isLoggedIn && config.FEATURE_FLAGS.dashboard">
        ...
        <router-link v-ripple class="sidebar__list-item"
                     @click.native="closeSidebar"
                     to="/ticket-creation"
                     tag="li" v-if="isLoggedIn &&
                                    accountTypeI === ACCOUNT_TYPES.syndicate &&
                                    config.FEATURE_FLAGS.ticketCreation"
        >
          <md-icon class="sidebar__list-item-icon">add_circle</md-icon>
          <span class="md-list-item-text">{{ i18n.lbl_create_ticket() }}</span>
        </router-link>
        ...
        <router-link v-ripple class="sidebar__list-item"
          ...
          <span class="md-list-item-text">{{ i18n.lbl_manager_ticket() }}</span>
        </router-link>
